...
def _inverse_logp_euler(
    ...
    def body(i, state):
        ...
        def f(x_in):
            return apply_fn(params, t_curr, x_in, None, train=False, method="calc_b")

        b, vjp_fn = jax.vjp(f, x)

        rng_key, eps_key = jax.random.split(rng_key)
        eps = jax.random.rademacher(eps_key, shape=x.shape, dtype=x.dtype)

        jte = vjp_fn(eps)[0]
        div = jnp.sum(jte * eps, axis=tuple(range(1, x.ndim)))

        x = x + dt * b
        delta_logp = delta_logp - dt * div
        # print the mean of div for debugging
...
def parse_args():
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="/data/user_data/xinyueai/flow-maps/celeba-flow/celeba_paper_lsd_13.pkl",
    )
